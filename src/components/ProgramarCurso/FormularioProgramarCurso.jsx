...
import {actualizarEspecifico,agregar,consultaById } from '../../utils/ConexionAPI'
...
const FormularioProgramarCurso = ({accion,programarCurso,actualizarListado,setProgramarCurso,cursoListado,periodoListado}) => {
    ...
    useEffect(() => {
        actualizarListadoAlumno(programarCurso.periodoId);
        setAlumnoListadoFinal(programarCurso.alumnos);
     }, [] )
 
     const actualizarListadoAlumno = async(sIdPeriodo) => {
        console.log(sIdPeriodo);
        if(sIdPeriodo === ''){
            return;
        }
        consultaById('periodo/listadoAlumnosByPeriodo/',sIdPeriodo).then((jsListado) => {
    ...
    return(
        <Formik
            ...
            onSubmit={(values,e) => {
                ...
                programarCurso.fechaInicioCurso = moment.utc(values.fechaInicioCurso,'YYYY-MM-DD').format("YYYY-MM-DD HH:mm");
                ...
                if(accion === 2){
                    agregar('programarCurso/createProgramarCurso',programarCurso).then( () => {
                        swal({
                    ...
                    actualizarEspecifico('programarCurso/actualizarAlumno',programarCurso).then( () => {
                        swal({
        ...
        <Form className="informacionUniversidad"
            ...
            <Row className="mb-3">
                <Form.Group as={Col}>
                    ...
                    <Form.Control as="select" name="cursoId" id="cursoId" value={values.cursoId}
                     ...
                     isInvalid={!!touched.cursoId && !!errors.cursoId}>
                        ...
                        { cursoListado.map( (consultaCursoListado,index) => (
                            <option key={index} value={consultaCursoListado._id}>{consultaCursoListado.nombreCurso}</option>
                        ))}
                    </Form.Control>
                ...
                <Form.Group as={Col}>
                    ...
                    <Form.Control  placeholder="Fecha Inicio" name="fechaInicioCurso" id="fechaInicioCurso" 
                        type="date"
                        value={values.fechaInicioCurso} onChange={handleChange} onBlur={handleBlur}
                        isInvalid={!!touched.fechaInicioCurso && !!errors.fechaInicioCurso}
                        />
                    <Form.Control.Feedback type="invalid">
                        {errors.fechaInicio}
                    </Form.Control.Feedback>
                ...
                <Form.Group as={Col}>
                    ...
                    <Form.Control as="select" name="periodoId" id="periodoId" value={values.periodoId}
                     onChange={handleChange} onBlur={handleBlur}
                     onChange={(e) => // call the built-in handleChange for formik
                        {handleChange(e)
                        actualizarListadoAlumno(e.currentTarget.value)}
                    } onBlur={handleBlur}
                     isInvalid={!!touched.periodoId && !!errors.periodoId}>
                        <option key={'0'} value="">--Seleccione un registro--</option>
                        { periodoListado.map( (consultaPeriodoId,index) => (
                            <option key={index} value={consultaPeriodoId._id}>{consultaPeriodoId.periodo}</option>
                        ))}
                    </Form.Control>
                    <Form.Control.Feedback type="invalid">
                        {errors.cursoId}
            ...
            <Row className="mb-3">
                ...
                <Form.Group as={Col}>
                    ...
                    <Form.Control as="select" name="tipoCurso" id="tipoCurso" 
                        value={values.tipoCurso} onChange={handleChange} onBlur={handleBlur}>
                ...
                <Form.Group as={Col}>
                    ...
                    <Form.Control  placeholder="Precio Final" name="precioFinal" id="precioFinal" 
                        value={values.precioFinal} onChange={(e) => // call the built-in handleChange for formik
                            {handleChange(e)
                            actualizarPrecioTotal(e.currentTarget.value,alumnoListadoFinal.length)}
                        } onBlur={handleBlur}
                ...
                <Form.Group as={Col}>
                    ...
                    <Form.Control  placeholder="Precio Total" name="precioTotal" id="precioTotal" 
                        value={precioTotal} onChange={handleChange} onBlur={handleBlur} disabled
            ...
            <Row className="mb-3">
                <Pagination>
                    <Pagination.Item onClick={() => eliminarRegistroTabla(values.precioFinal)}>{`<`}</Pagination.Item>
                    <Pagination.Item onClick={() => agregarRegistroTabla(values.precioFinal)}>{`>`}</Pagination.Item>
                </Pagination>
            </Row>
            <Row className="mb-3">
                <Col>
