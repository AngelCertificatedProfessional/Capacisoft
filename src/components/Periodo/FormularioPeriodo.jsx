...
const FormularioPeriodo = ({ accion, periodo, actualizarListado }) => {
  ...
  const bajaAlumno = () => {
    const objIndex = alumnoListadoFinal.findIndex(
      (alumnoListadoFinal) => alumnoListadoFinal._id == seleccionadoFinal
    );
    if (objIndex === -1) {
      return;
  ...
  return (
    <Formik
      ...
      onSubmit={(values, e) => {
        ...
        periodo.fechaInicio = moment
          .utc(values.fechaInicio, 'YYYY-MM-DD')
          .format('YYYY-MM-DD HH:mm');
        periodo.fechaFinal = moment
          .utc(values.fechaFinal, 'YYYY-MM-DD')
          .format('YYYY-MM-DD HH:mm');
    ...
    >
      {({
        ...
        >
          ...
          <h2>Agregar Alumnos al periodo</h2>
          <Row className="mb-3">
            <Pagination>
              <Pagination.Item
